...
@section('content')
    <div class="card shadow mb-4">
        <div class="card-header py-3 mb-3">
            <span class="h5 m-0 font-weight-bold text-primary">Edit Pegawai</span>
        </div>
        <div class="card-body">
            <form class="user" method="POST" action="{{ route('update-pegawai', ['id' => $pegawai->id]) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select class="form-control" id="jabatan" required name="jabatan">
                                <option value="Pegawai" {{ $pegawai->jabatan == 'Pegawai' ? 'selected' : '' }}>Pegawai
                                </option>
                ...
                <div class="form-group">
                    ...
                    <textarea name="alamat" id="alamat" rows="5" class="form-control" required>{{ $pegawai->alamat }}</textarea>
                    @error('alamat')
                        <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
                </div>
