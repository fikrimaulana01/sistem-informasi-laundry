...
@section('content')
    <div class="card shadow mb-4">
        <div class="card-header py-3 mb-3">
            <span class="h5 m-0 font-weight-bold text-primary">Tambah Pesanan</span>
        </div>
        <div class="card-body">
            <form class="user" method="POST" action="{{ route('store-pesanan') }}">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="id_layanan" required name="id_layanan">
                        @foreach ($layanans as $layanan)
                            <option value="{{ $layanan->id }}" {{ old('id_layanan') == $layanan->nama ? 'selected' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <div>
                        <span>Jika berat pakaian berkoma harap pakai titik contoh : <span class="text-info">2.5</span></span>
                    </div>
                    @error('berat')
                        <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
